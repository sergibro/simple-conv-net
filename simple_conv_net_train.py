...
class SimpleConvNet(nn.Module):
    def __init__(self, device, _type=''):
...
def main(args):
    ...
    model = SimpleConvNet(device, args.type)
    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum)
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer, epoch)
        test(args, model, device, test_loader)
    if (args.save_model):
        torch.save(model.state_dict(), f"mnist_cnn{args.type}.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', type=str, default='', metavar='T',
                        help='type of training mode - scalar/vector or in-built (default)')
    parser.add_argument('--batch-limit', type=int, default=0, metavar='N',
                        help='how many batches to limit for faster training tests (0 - no limit)')
